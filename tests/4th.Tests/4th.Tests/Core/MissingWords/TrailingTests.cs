...
namespace Forth.Tests.Core.MissingWords
{
    public class TrailingTests
    {
        ...
        [Fact]
        public async Task Trailing_NegativeLength_Throws()
        {
            var f = New();
            var ex = await Assert.ThrowsAsync<Forth.Core.ForthException>(() => f.EvalAsync("PAD -1 -TRAILING"));
            Assert.Equal(Forth.Core.ForthErrorCode.TypeError, ex.Code);
        }

        [Fact]
        public async Task Trailing_ZeroLength()
        {
            var f = New();
            Assert.True(await f.EvalAsync("PAD 0 -TRAILING"));
            Assert.Equal(2, f.Stack.Count);
            Assert.IsType<long>(f.Stack[0]); // address
            Assert.Equal(0L, (long)f.Stack[1]);
        }
        ...
        [Fact]
        public async Task Trailing_SingleSpace()
        {
            var f = New();
            Assert.True(await f.EvalAsync("S\" \" -TRAILING"));
            Assert.Equal(2, f.Stack.Count);
        ...
        [Fact]
        public async Task Trailing_LeadingAndTrailingSpaces()
        {
            var f = New();
            Assert.True(await f.EvalAsync("S\"  hello  \" -TRAILING"));
            Assert.Equal(2, f.Stack.Count);
            Assert.IsType<long>(f.Stack[0]); // address
            Assert.Equal(6L, (long)f.Stack[1]); // " hello" length after trimming
        }
    }
}