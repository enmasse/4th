...
internal static partial class CorePrimitives
{
    ...
    [Primitive("2R@", HelpString = "2R@ ( -- a b ) - copy two items from return stack")]
    private static Task Prim_2RAt(ForthInterpreter i)
    {
        if (i.RCount < 2) throw new ForthException(ForthErrorCode.StackUnderflow, "Return stack underflow in 2R@");
        var b = i.RTop();
        var a = i.RNth(1);
        i.Push(a);
        i.Push(b);
    ...
    private static Task Prim_RAt(ForthInterpreter i)
    {
        if (i.RCount == 0) throw new ForthException(ForthErrorCode.StackUnderflow, "Return stack underflow in R@");
        var top = i.RTop();
        i.Push(top);
        return Task.CompletedTask;
    }
